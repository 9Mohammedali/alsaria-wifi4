...
async function fetchCategories() {
  ...
  // إذا لم يوجد مستند فئات، أنشئ المستند الافتراضي
  if (!found) {
...
async function updateProfitIcon() {
  ...
  if (!isOwner && userData.role === "user") {
    // احسب أرباح المستخدم (20% من مبيعاته)
    let total = 0;
...
function renderSalesButtons() {
  ...
  // تحديث قائمة الفئات في نافذة الإدخال
...
// ========== تحويل الرصيد ==========
document.getElementById("transfer-btn-modal").onclick = async () => {
...
async function renderTransferReport() {
  ...
  let html = `<table style="font-size:15px;"><tr><th>رقم الحساب</th><th>الإسم</th><th>البريد</th><th>عدد العمليات</th><th>إجمالي الرصيد المحول</th><th>عرض التفاصيل</th></tr>`;
...
// ====== أزرار النوافذ ======
document.getElementById("transfer-btn-head").onclick = () => {
